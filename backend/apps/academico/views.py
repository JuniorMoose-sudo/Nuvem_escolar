...
class MateriaViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return Materia.objects.filter(escola=self.request.user.escola)
...
class TurmaViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if user.tipo_usuario == Usuario.TipoUsuario.ADMIN_ESCOLA:
            return Turma.objects.filter(escola=user.escola)
        if user.tipo_usuario == Usuario.TipoUsuario.PROFESSOR:
            # Professor vê apenas as turmas às quais está vinculado.
            return Turma.objects.filter(vinculos_professores__professor=user.perfil_professor)
...
class AlunoViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        user = self.request.user
        if user.tipo_usuario == Usuario.TipoUsuario.ADMIN_ESCOLA:
            return Aluno.objects.filter(escola=user.escola)
        if user.tipo_usuario == Usuario.TipoUsuario.PROFESSOR:
            ...
            return Aluno.objects.filter(turma__id__in=turmas_ids)
        if user.tipo_usuario == Usuario.TipoUsuario.RESPONSAVEL:
            # Responsável vê apenas seus filhos.
            return Aluno.objects.filter(vinculos_responsaveis__responsavel=user.perfil_responsavel)
